...
async function readIds(txtPath) {
  ...
  if (lines.length === 0) throw new Error(`Nenhum ID ou URL em: ${txtPath}`);

  // Processa cada linha - pode ser URL completa ou ID
  const processedLines = lines.map(line => {
    if (isValidUrl(line) && isMercadoLivreUrl(line)) {
      // É uma URL válida do Mercado Livre
      return { type: 'url', value: line };
    } else {
      // É provavelmente um ID
...
async function dismissPopups(page) {
  const texts = ["Mais tarde", "Agora não", "Fechar"];
...
async function openByRule(page, item) {
  let url;
  let tried = [];
  ...
  if (item.type === 'url') {
    ...
    url = item.value;
    console.log(`🔎 [URL] Tentando abrir: ${url}`);
  } else {
    // É um ID, precisa construir a URL
    url = buildUrlByRule(item.value);
    if (!url) return { ok: false, reason: "Prefixo de ID não suportado", tried: [] };
    console.log(`🔎 [${item.value}] Tentando abrir: ${url}`);
  }
  ...
  try {
    ...
    console.log(`❌ Erro ao carregar página: ${e.message}`);
  }
  return { ok: false, reason: "Não abriu como produto (sem título)", tried };
}
...
async function getImageUrlsFromLightbox(page, expectedCountHint = 0) {
  ...
  for (const sel of clickSelectors) {
    const el = await page.$(sel);
    if (el) { await el.click({ delay: 60 }); clicked = true; break; }
  }
  if (!clicked) {
  ...
  for (let i = 0; i < maxIters; i++) {
    ...
    for (const sel of nextSelectors) {
      ...
      if (btn) { await btn.click({ delay: 50 }); advanced = true; break; }
    }
  ...
  try {
    ...
    if (closeBtn) await closeBtn.click({ delay: 50 }); else await page.keyboard.press("Escape");
  } catch {}
...
async function getImageUrlsFromThumbs(page, expectedCountHint = 0) {
  ...
  if (!thumbs.length) {
    ...
    for (let i = 0; i < thumbs.length; i++) {
      try { await thumbs[i].click({ delay: 40 }); } catch {}
...
async function getImageUrls(page, expectedCountHint = 0) {
  await page.evaluate(() => {
    const g = document.querySelector("#ui-pdp-main-container .ui-pdp-gallery, #ui-pdp-main-container .ui-pdp-image");
...
// =============== FLUXO POR ITEM ===============
async function scrapeOne(page, item, idx, total) {
  const identifier = item.type === 'url' ? 'URL' : item.value;
  console.log(`\n=== [${idx + 1}/${total}] ${item.type === 'url' ? 'URL' : 'ID'} ${identifier} ===`);
  ...
  const opened = await openByRule(page, item);
  if (!opened.ok) {
    return {
      item_id: item.type === 'url' ? 'URL' : item.value,
      item_type: item.type,
      erro: `Falha: ${opened.reason}`,
      tentativas: opened.tried?.join(" | ") || ""
    };
  }
...
function toCsv(rows) {
  ...
  const preferred = ["item_id","item_type","nome_ad","descricao","ficha_tecnica","vendas","qtd_imagens","imagens_urls","preco","compatibilidade","permalink","erro","tentativas"];
...
async function main() {
  const items = await readIds(DEFAULT_IDS_FILE);
  ...
  try {
    for (let i = 0; i < items.length; i++) {
      const item = items[i];
      try {
        const out = await scrapeOne(page, item, i, total);
        results.push(out);
      } catch (e) {
        console.log(`❌ Erro no scrape: ${e.message}`);
        results.push({
          item_id: item.type === 'url' ? 'URL' : item.value,
          item_type: item.type,
          erro: String(e)
