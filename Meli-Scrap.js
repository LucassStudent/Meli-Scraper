...
function buildUrlByRule(itemId) {
    ...
    // Remove qualquer hífen existente no ID
    const cleanId = id.replace(/-/g, '');

    if (cleanId.startsWith("MLBU")) {
        return `http://mercadolivre.com.br/aaa/up/${cleanId}`;
    }
    if (cleanId.startsWith("MLB")) {
        // Para IDs mais curtos (provavelmente mais antigos) usa formato /p/
        // Para IDs mais longos (provavelmente mais novos) usa formato com hífen
        const numericPart = cleanId.slice(3);
        const isShortId = numericPart.length <= 8;

        return isShortId ?
            `http://mercadolivre.com.br/aaa/p/${cleanId}` :
            `https://produto.mercadolivre.com.br/MLB-${numericPart}`;
    }
    return null;
}
